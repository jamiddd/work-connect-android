...
): PagedListAdapter<SimpleMessage, RecyclerView.ViewHolder>(GenericComparator(SimpleMessage::class.java)) {
	override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
		return when (viewType) {
			1 -> GridImageViewHolder.newInstance(parent, actContext)
			2 -> LinksViewHolder.newInstance(parent, actContext)
			3 -> DocumentViewHolder.newInstance(parent, actContext, viewModel)
			else -> GridImageViewHolder.newInstance(parent, actContext)
		}
	}
