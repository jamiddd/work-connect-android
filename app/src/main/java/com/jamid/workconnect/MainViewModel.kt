...
class MainViewModel(application: Application) : AndroidViewModel(application) {
	...
	private val _miniUser = MutableLiveData<UserMinimal>().apply { value = null }
	...
	val mediaDownloadResult = repo.mediaDownloadResult

//	val allRecentSearches = repo.allRecentSearches
...
//        }
	...
	fun chatChannelMessages(chatChannel: ChatChannel, externalImagesDir: File, externalDocumentsDir: File): Flow<PagingData<SimpleMessage>> {
		return Pager(PagingConfig(pageSize = 30, initialLoadSize = 50, prefetchDistance = 10),
				remoteMediator = MessagesRemoteMediator(repo, chatChannel, externalImagesDir, externalDocumentsDir)) {
	...
	fun searchPosts(query: String, type: String): Flow<PagingData<Post>> {
		val postSource = PostSource.Search(Firebase.firestore.collection(POSTS)
	...
	fun searchUsers(query: String): Flow<PagingData<User>> {
		val userSource = UserSource.Search(Firebase.firestore.collection(
			USERS).whereArrayContainsAny(INDICES, listOf(
			query,
			query.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() },
			query.replaceFirstChar { it.lowercase(Locale.ROOT) },
			query.uppercase(Locale.ROOT), query.lowercase(Locale.ROOT)
...
*/
	...
	fun userFollowers(otherUser: User, q: String? = null): Flow<PagingData<User>> {
		Log.d(FOLLOWERS, "User follower function ..")
		return if (q == null) {
			Log.d(FOLLOWERS, "User follower function .. query is null, otherUserId -> ${otherUser.id}")

			val userSource = UserSource.Follower(Firebase.firestore.collection(
				USERS).document(otherUser.id).collection(FOLLOWERS).document(otherUser.id).collection(USERS).orderBy(NAME, Query.Direction.ASCENDING), q, otherUser)
			...
			Log.d(FOLLOWERS, "User follower function .. query is not null")
			val userSource = UserSource.Follower(Firebase.firestore.collection(
				USERS).document(otherUser.id).collection(FOLLOWERS)
				.document(otherUser.id).collection(USERS)
				.whereArrayContainsAny(
					INDICES,
					listOf(q, q.capitalize(Locale.ROOT), q.decapitalize(Locale.ROOT), q.toUpperCase(Locale.ROOT), q.toLowerCase(Locale.ROOT))
	...
	fun userFollowings(otherUser: User, q: String? = null) : Flow<PagingData<User>> {
		return if (q == null) {
			val userSource = UserSource.Following(Firebase.firestore.collection(
				USERS).document(otherUser.id).collection(FOLLOWINGS).document(otherUser.id).collection(USERS).orderBy(NAME, Query.Direction.ASCENDING), q, otherUser)
			...
			val userSource = UserSource.Following(Firebase.firestore.collection(
				USERS).document(otherUser.id).collection(FOLLOWINGS)
				.document(otherUser.id).collection(USERS)
				.whereArrayContainsAny(
						INDICES,
						listOf(q, q.capitalize(Locale.ROOT), q.decapitalize(Locale.ROOT), q.toUpperCase(Locale.ROOT), q.toLowerCase(Locale.ROOT))
	...
	// TODO("Let repository return the data, so that firebase functions should only reside in repository")
	...
	fun postsFlow(tag: String? = null): Flow<PagingData<Post>> {
		...
		return if (tag != null) {
			val tags = listOf(tag,
				...
				tag.replaceFirstChar { it.lowercase(Locale.ROOT) }, tag.uppercase(Locale.ROOT), tag.lowercase(
					Locale.ROOT
				)
			)
			val postSource = PostSource.FeedWithTags(Firebase.firestore.collection(POSTS).whereArrayContainsAny(
				TAGS,
				tags
			), tags)
			Pager(PagingConfig(20)) {
			...
			val postSource = if (currentUser != null) {
				...
				if (finalList.size < 2) {
					...
					PostSource.FeedWithFollowings(Firebase.firestore.collection(POSTS).whereIn(UID, finalList), currentUser)
				}
			} else {
				PostSource.FeedRandom(Firebase.firestore.collection(POSTS))
			}
	...
	fun otherUserProjectsFlow(otherUser: User): Flow<PagingData<Post>> {
		val postSource = PostSource.FeedWithOtherUserAndType(Firebase.firestore.collection(POSTS)
	...
	fun otherUserCollaborationsFlow(otherUser: User): Flow<PagingData<Post>> {
		val postSource = PostSource.FeedWithOtherUserCollaborations(Firebase.firestore.collection(POSTS)
	...
	fun setChatChannelListeners() = viewModelScope.launch (Dispatchers.IO) {
		repo.setChatChannelListeners()
	}

	fun getContributorsForAllChannels() {
		repo.getContributorsForAllChannels()
	}
	...
	fun setChannelMessagesListener(externalImagesDir: File, externalDocumentsDir: File, chatChannel: ChatChannel)  = viewModelScope.launch (Dispatchers.IO) {
		repo.setChannelMessagesListener(externalImagesDir, externalDocumentsDir, chatChannel)
	}
	...
	fun getQueryPosts(postsTag: String): Flow<PagingData<Post>> {
		val postSource = PostSource.TagPosts(Firebase.firestore.collection(POSTS).whereArrayContains(TAGS, postsTag), postsTag)
	...
	fun topProjectsFlow(): Flow<PagingData<Post>> {
		val postSource = PostSource.FeedRandom(Firebase.firestore.collection(POSTS)
	...
	fun getRandomUsers(): Flow<PagingData<User>> {
		val userSource = UserSource.Random(Firebase.firestore.collection(USERS)
	...
	fun getMessagesSender(senderId: String): LiveData<User> {
		return repo.getMessageSender(senderId)
	}

	fun downloadImage(message: SimpleMessage, externalDir: File) {
		repo.downloadImage(message, externalDir)
	}
