...
class FirebaseUtilityImpl(private val scope: CoroutineScope, database: WorkConnectDatabase) :
	...
	var contributorListenerRegistration: ListenerRegistration? = null
	var chatChannelListenerRegistration: ListenerRegistration? = null

	private val channelContributorsListeners = mutableMapOf<String, ListenerRegistration>()
	...
	private fun addUserListener(firebaseUser: FirebaseUser) {
		...
		addDocumentListener<UserPrivate>(
			currentUserRef.collection(PRIVATE).document(firebaseUser.uid)
		)
	}
	...
	override fun getToken() {
		FirebaseMessaging.getInstance()
			.token.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get identity token." + errorSuffix + it.localizedMessage!!
				)
			}
	}
					...
					Firebase.firestore.collection(USERS).document(currentUser.uid).get()
						.addOnSuccessListener { userDoc ->
							if (userDoc.exists()) {
								...
								_networkErrors.postValue(Exception("User document doesn't exist."))
							}
	...
	override fun uploadCurrentUser(user: User) {
		FirebaseMessaging.getInstance()
			.token.addOnSuccessListener { token ->
				...
				Log.d(BUG_TAG, "Received the token - $token")
...
//				updateRegistrationToken(it)
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get token from server" + errorSuffix + it.localizedMessage!!
				)
			}
	...
	override fun updateCurrentUser(userMap: Map<String, Any?>) {
		...
		db.runBatch {
			...
			/*for (i in userDetail.notificationReferences.indices) {
				val notificationRef =
					db.collection(USERS).document(userDetail.notificationReferences[i]).collection(
						NOTIFICATIONS
					).document(notifications[i])
...
//                    val data = taskSnapshot.bytesTransferred
	...
	fun acceptProjectRequest(notification: SimpleNotification) {
		...
		db.runBatch {
			...
			scope.launch(Dispatchers.IO) {
				insertUsersWithFilter(listOf(localSender))
	...
	override fun joinProject(post: Post) {
		...
		val senderRequestRef =
			db.collection(USERS).document(currentUser.id).collection(REQUESTS).document(requestId)
	...
	// the state before changes
	override fun onLikePressedWithoutCaching(post: Post): Post {
		val userId = currentLocalUser.value!!.id
		val userMap = mutableMapOf<String, Any>()
		...
		db.runBatch {
			...
			Log.d(BUG_TAG, "Liked a post with title: ${post.title}")
		}
		return post
	}
	...
	override fun onDislikePressedWithoutCaching(post: Post): Post {
		val userId = currentLocalUser.value!!.id
		val userMap = mutableMapOf<String, Any>()
		...
		db.runBatch {
			...
			Log.d(BUG_TAG, "Disliked post with title: ${post.title}")
		}
		return post
	}
	...
	override fun onPostSavedWithoutCaching(post: Post): Post {
		val userId = currentLocalUser.value!!.id
		val currentUserPrivateRef =
			db.collection(USERS).document(userId).collection(PRIVATE).document(userId)
		val task = if (post.postLocalData.isSaved) {
		...
		task.addOnSuccessListener {
			...
			Log.d(BUG_TAG, "Saved post with title: ${post.title}")
		}.addOnFailureListener {
			Log.e(BUG_TAG, errorPrefix + "save a post." + errorSuffix + it.localizedMessage!!)
		}
		return post
	...
	fun onFollowPressed(currentUser: User, otherUser: User) {
		val currentUserPrivateRef =
			db.collection(USERS).document(currentUser.id).collection(PRIVATE)
				.document(currentUser.id)
		val otherUserRef =
			db.collection(USERS).document(otherUser.id).collection(PRIVATE).document(otherUser.id)
		...
		val followersRef =
			db.collection(USERS).document(otherUser.id).collection(FOLLOWERS).document(otherUser.id)
	...
	fun <T : Any> getPostsOnStart(
		...
		query.get()
			.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get a bunch of post on start." + errorSuffix + it.localizedMessage!!
				)
			}
	}
	...
	override fun uploadMessageMedia(message: SimpleMessage, chatChannel: ChatChannel) {
		...
		if (message.type == IMAGE) {
			randomName =
				"Image_" + SimpleDateFormat("yyyyMMdd_HHmmss", Locale.UK).format(message.createdAt)
		...
		mediaRef?.let { ref ->
			ref.putFile(message.content.toUri())
				...
				.addOnFailureListener {
					Log.e(
						BUG_TAG,
						errorPrefix + "upload a message media." + errorSuffix + it.localizedMessage!!
					)
				}
		}
	...
	override fun getContributorsForPost(channelId: String) {
		db.collection(CHAT_CHANNELS).document(channelId).collection(CONTRIBUTORS).limit(10).get()
			.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get contributors for a post." + errorSuffix + it.localizedMessage!!
				)
			}
	}
	...
	override fun getChannelContributors(
		...
		query.limit(pageSize)
			...
			.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get channel contributors." + errorSuffix + it.localizedMessage!!
				)
			}
	}
	...
	private suspend fun insertUsersWithFilter(users: List<User>, item: Any? = null) {
		Log.d(BUG_TAG, "Inserting users .. ${users.size}")
		if (item != null) {
			// some filters needed
			if (item is ChatChannel) {
				item.lastMessage?.sender = users.find {
					it.id == item.lastMessage?.senderId
				}!!
				chatChannelDao.updateItem(item)
			}
		}
	...
	fun <T> getItems(
		...
		query.get()
			.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get ${clazz::getCanonicalName} items for lists." + errorSuffix + it.localizedMessage!!
				)
			}
	}
	...
	fun <T : Any> getObject(
		...
		ref?.let {
			ref.document(objectId).get()
				.addOnSuccessListener {
					...
					Log.e(
						BUG_TAG,
						errorPrefix + "get object of unknown type." + errorSuffix + it.localizedMessage!!
					)
				}
		}
		...
		if (ref == null) {
			Log.e(
				BUG_TAG,
				errorPrefix + "create a reference." + errorSuffix + "Probably ref is null."
			)
		}
			...
			for (j in 0 until batchCount) {
				...
				db.collection(POSTS)
					...
					.addOnSuccessListener {
						...
						Log.e(
							BUG_TAG,
							errorPrefix + "download saved posts." + errorSuffix + it.localizedMessage!!
						)
					}
	...
	fun getNotificationFromFirebase(notificationId: String) {
		...
		db.collection(USERS).document(currentUser.id)
			...
			.addOnSuccessListener {
				...
				Log.e(
					BUG_TAG,
					errorPrefix + "get notifications." + errorSuffix + it.localizedMessage!!
				)
			}
	}
	...
	suspend fun getPagedRequests(query: Query, limit: Int, startAfter: DocumentSnapshot?): Result<QuerySnapshot> {
		return if (startAfter != null) {
			val task = query.startAfter(startAfter).limit(limit.toLong()).get()
			Result.Success(task.await())
		} else {
	...
	suspend fun setChannelMessagesListener(externalImagesDir: File, externalDocumentsDir: File, chatChannel: ChatChannel) {
		setMessagesListener(externalImagesDir, externalDocumentsDir, chatChannel)
	}
